...
class SAR(nn.Module):
    ...
    def forward(self, x, rendition_mask=None):
        ...
        for _ in range(self.steps):
            outputs, ema, reset_flag = forward_and_adapt_sar(x, self.model, self.optimizer, self.margin_e0, self.reset_constant_em, self.ema, rendition_mask)
            if reset_flag:
                self.reset()
    ...
    def forward_only(self, x, rendition_mask=None):
        ...
        if rendition_mask is None:
            ...
            outputs = self.model(x)[:, rendition_mask]
        return outputs
...
def forward_and_adapt_sar(x, model, optimizer, margin, reset_constant, ema, rendition_mask=None):
    ...
    if rendition_mask is None:
        ...
        outputs = model(x)[:, rendition_mask]
    # adapt
...
def collect_params(model):
    ...
    for nm, m in model.named_modules():
        ...
        if isinstance(m, (nn.BatchNorm2d, nn.LayerNorm, nn.GroupNorm)):
            m.weight = nn.Parameter(torch.ones(m.num_features, device='cuda'))
            m.bias = nn.Parameter(torch.zeros(m.num_features, device='cuda'))            
