...
from dataset.cifar100c import CIFAR100C
...
te_transforms_cifar10_c = transforms.Compose([
                                    transforms.ToTensor()])

te_transforms_mnist = transforms.Compose([
                                    transforms.Pad(2),
                                    transforms.Lambda(lambda x: x.convert("RGB")),
                                    transforms.ToTensor()])

te_transforms_mnist_m = transforms.Compose([
                                    # transforms.Pad(2),
                                    # transforms.Lambda(lambda x: x.convert("RGB")),
                                    transforms.ToTensor()])

te_transforms_usps = transforms.Compose([
...
def prepare_test_data(args, use_transforms=True):	
    if args.corruption == 'original':
        ...
        te_transforms_local = te_transforms if use_transforms else None
    elif args.corruption=='visda-c':
        te_transforms_local = te_transforms if use_transforms else None          
    else:
        assert False, NotImplementedError
    if 'CIFAR-10-C' in args.data_corruption:
        teset = CIFAR10C(
            os.path.join(args.data_corruption),
            cname=args.corruption,
            transform=te_transforms_cifar10_c,
            severity=args.level)
    elif 'CIFAR-100-C' in args.data_corruption:
        teset = CIFAR100C(
            os.path.join(args.data_corruption),
            cname=args.corruption,
            transform=te_transforms_cifar10_c,
            severity=args.level)
    elif 'USPS' in args.data_corruption:
        teset = datasets.USPS(os.path.join(args.data_corruption), train=False, download=True,
                       transform=te_transforms_usps)
    elif 'SVHN' in args.data_corruption:
        teset = datasets.SVHN(os.path.join(args.data_corruption), split='test', download=True,
                       transform=te_transforms_cifar10_c)        
    elif 'MNIST-M' in args.data_corruption:
            teset = datasets.ImageFolder(os.path.join(args.data_corruption,'testing'),
                        transform=te_transforms_mnist_m)    
    elif 'MNIST' in args.data_corruption:
        teset = datasets.MNIST(os.path.join(args.data_corruption), train=False, download=True,
                       transform=te_transforms_mnist)
    elif not hasattr(args, 'corruption') or args.corruption == 'original':
        ...
        assert 'imagenet-r' in args.data_corruption
        ...
        teset.imagenet_r_mask = [wnid in imagenet_r_wnids for wnid in all_wnids]
        

        print('Test on imagenet-rendition')
    elif args.corruption=='sketch':
        assert 'sketch' in args.data_corruption
        validdir = os.path.join(args.data_corruption)
        teset = SelectedRotateImageFolder(validdir, te_transforms_local, original=False, rotation=False,
                                                    rotation_transform=rotation_te_transforms)
        print('Test on imagenet-sketch')
    elif args.corruption=='visda-c':
        validdir = os.path.join(args.data_corruption)
        teset =  VisDAC(validdir,
                    transform=te_transforms_local)
        print('Test on visda-c')
    else:
